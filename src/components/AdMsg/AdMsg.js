...
class AdMsg extends Component {
    ...
    render() {
        ...
        let abstract = "A mobile application that allows users to covertly communi- cate through mobile \
        ...
        access to advertisement data packets. MITMProxy is the tool used in order to cap- ture packets and \
        ...
        comes included with MITMProxy. A Python script is fed into MITMDDump that scrapes user mes- sages from \
        incoming packets and saves them locally to the device. MITMProxy can run in a Linux environ- ment, the \
        ...
        avail- able on the app store which can emulate a Linux en- vironment on Android devices. AnLinux is \
        ...
        let howItWorks = "As proxied packets come flooding through MITM- Dump is able to go through packet \
        by packet and scrape out the content. A filtering function is set to capture all incoming packets \
        specifically filtering for advertise- ment data packets containing HTTP and JavaScript. A Python \
        ...
        from ad - vertisements are stored on the phone in sdcard storage.The messaging app then periodically \
        checks to see if any new messages have been captured and stored in the sd - card.When there are new \
        messages the user will be notified and has a chance to reply to the message.Out - going messages will \
        be queued until the user receives another advertisement packet that is able to carry the message back \
        by disguising the message as metadata which is sent back to the server.Users exchange ad - vertising \
        IDs when they start using the app to make targeting advertisements to their specific android de - \
        vice much easier and lower costs.In order to be able to run ads and message through them the user \
        ...
        In order to get advertise - ments displayed on your phone and be able to message you need to be \
        ...
        Mobile Advertisements are run globally and no coun- try has banned mobile advertising. This makes \
        it an easy target for covert communication because advertis- ing is so widely used and rarely \
        regulated. The mes- saging app would allow communication with those in countries where all other \
        ...
        not able to com- municate with before due to some sort of communica- tion barrier. This tool will be\
         ...
         it really hard scrape everyone’s adver- tisement for hidden messages. This means the cost of monitoring for \
